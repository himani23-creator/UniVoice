...
router.get("/:postId", (req: Request, res: Response) => {
  try {
    ...
    const comments = commentService.getCommentsByPost(req.params.postId, isAdmin);
...
router.post(
  ...
  (req: Request, res: Response) => {
    try {
      ...
      const comment = commentService.addComment({ complaintId, userId, body, isInternal });
...
router.patch("/:commentId", (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.userId ?? req.body.userId;
    const updated = commentService.editComment(req.params.commentId, req.body.body, userId);
...
router.delete("/:commentId", (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.userId ?? req.body.userId;
    const isAdmin = (req as any).user?.role === "admin";
    commentService.deleteComment(req.params.commentId, userId, isAdmin);
